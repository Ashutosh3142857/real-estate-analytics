...
def show_ad_performance_analytics():
    ...
    try:
        from utils.ad_platforms import get_all_ad_performance, get_ad_platforms
    except ImportError as e:
        logger.error(f"Error importing ad platform modules: {e}")
        st.error("Ad platform integration is not available.")
        # Fallback to sample data
        if 'marketing_data' not in st.session_state:
            st.session_state.marketing_data = generate_marketing_data()
        platforms_configured = False
    else:
        # Check if ad platforms are configured
        ad_platforms = get_ad_platforms()
        platforms_configured = any(platform.connected for platform in ad_platforms) if ad_platforms else False
        
        if not platforms_configured:
            st.warning("⚠️ No ad platforms are configured. Please set up ad platform credentials in API Settings to track campaign performance.")
            
            # Link to API settings
            st.markdown("[Go to API Settings to Configure Ad Platforms](/api_settings)")
            ...
            # Platform selector
            platform_options = ["All Platforms"] + [platform.display_name for platform in ad_platforms if platform.connected]
            selected_platform = st.selectbox("Select Platform", options=platform_options)
            
            # Fetch ad performance data
            try:
                if selected_platform == "All Platforms":
                    platform_names = [platform.name for platform in ad_platforms if platform.connected]
                else:
                    # Find platform name by display name
                    platform_obj = next((p for p in ad_platforms if p.display_name == selected_platform), None)
                    platform_names = [platform_obj.name] if platform_obj else []
                
                with st.spinner("Fetching ad performance data..."):
                    performance_data = get_all_ad_performance(
                ...
                if not performance_data or all(df.empty for df in performance_data.values()):
                    st.info("No ad performance data found for the selected period.")
                    # Fallback to sample data
                    if 'marketing_data' not in st.session_state:
                        st.session_state.marketing_data = generate_marketing_data()
                else:
                    # Process and display the data
                    # This will replace the sample data in session_state
                    marketing_data = {
                        'campaigns': [],
                        'channels': [],
                        'content_types': [],
                    ...
                    for platform_name, df in performance_data.items():
                        if not df.empty:
                            ...
                            for _, row in df.iterrows():
                                ...
                                marketing_data['campaigns'].append(campaign_data)
                                
                                # Add to daily performance (for time series)
                                marketing_data['daily_performance'].append({
                                    'date': row['date'],
                                    'platform': platform_name,
                ...
                st.error(f"Error fetching ad performance data: {str(e)}")
                # Fallback to sample data
                if 'marketing_data' not in st.session_state:
                    st.session_state.marketing_data = generate_marketing_data()
